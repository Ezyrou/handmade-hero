...
pub fn wWinMain(instance: w.HINSTANCE, _: ?w.HINSTANCE, _: [*:0]u16, _: i32) i32 {
    ...
    //
    // This system seems to be pretty flexible: window procedures can be
    // called directly by Windows for messages that should not wait for the
    // queue, queued messages that don't need to be dispatched to any window
    // can be handled in the message loop, the others can be dispatched to
    // their respective window procedure, and there is separation of concerns
    // where each window class has its own window procedure so they can handle
    // their own logic.
...
// The calling convention needs to be specified explicitly here because
// this is a function defined in Zig that will be called by Windows. It
// thus needs to use the winapi calling convention.
//
...
// this callback, we would need to handle messages such as WM_CLOSE or
...
fn wndProc(window: w.HWND, message: u32, wparam: w.WPARAM, lparam: w.LPARAM) callconv(w.WINAPI) w.LRESULT {
    ...
    switch (message) {
        ...
        // already been removed from the screen, so the point of the
        ...
        w.WM_PAINT => {
            ...
            if (w.BeginPaint(window, &paint)) |device_context| {
                ...
                // Paint a rectangle. In our case, the whole update region.
